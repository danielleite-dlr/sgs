...
import { TIME_SLOTS } from "../mocks/schedule.mock";
import {
  AppointmentsQuery,
...
const APPT_STATUSES = [
  {
    id: "absent",
    label: "Ausência de profissional",
    color: "#D9D9D9",
    textOnColor: "#5C5C5C",
  },
  {
...
function appointmentToSchedule(
  ...
  return {
    ...
    categoryColor:
      PROFESSIONAL_COLORS[
        Math.abs(appointment.service.name.length) % PROFESSIONAL_COLORS.length
      ],
    startHour: startsAt.getHours(),
    startMinute: startsAt.getMinutes(),
    durationMinutes: Math.round(
      (new Date(appointment.endsAt).getTime() - startsAt.getTime()) / 60000,
    ),
...
function InlineCalendar({ selected, onSelect }: InlineCalendarProps) {
  ...
  for (let i = 0; i < startWeekday; i++)
    cells.push({ date: null, isCurrent: false });
  ...
  return (
    <aside className="flex flex-col gap-md">
              ...
              <button
                ...
                className={cn(
                  ...
                  active
                    ...
                    : "opacity-50 border-neutral-200 bg-white text-neutral-600",
                )}
                style={
                  active
      ...
      <CollapsibleSection title="Exibição da agenda">
        <Label className="text-xs text-neutral-600 mb-xs block">
          Mostrar folga:
        </Label>
        <RadioGroup
...
function SelecaoProfissionaisModal({
  ...
  function toggleAll() {
    ...
    else setLocalVisible(new Set(professionals.map((p) => p.id)));
  }
...
function AppointmentModal({
  ...
  const { data: clientsData } = useQuery<ClientsListResult>(ClientsListQuery, {
    variables: { search: clientSearch || null, limit: 10, offset: 0 },
    skip: !open,
  });
  const { data: servicesData } = useQuery<ServicesQueryResult>(ServicesQuery, {
    skip: !open,
  });
  const [createAppointment, { loading }] = useMutation<CreateAppointmentResult>(
    CreateAppointmentMutation,
  );

  useEffect(() => {
    if (open) {
      setProfessionalId(initialTime?.profId ?? "");
      setTime(
        initialTime
          ? `${String(initialTime.hour).padStart(2, "0")}:${String(initialTime.minute).padStart(2, "0")}`
          : "",
  ...
  const selectedClient = clientsData?.clients.rows.find(
  ...
  async function handleSave() {
    if (
      ...
      toast.error("Preencha cliente, profissional, serviço, data e hora.");
      return;
    }
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-[640px] p-0 max-h-[90vh] flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto px-md py-md space-y-md">
          <div>
            ...
            {clientSearch && !clientId && (
              <div className="mt-xs max-h-32 overflow-y-auto rounded-md border border-neutral-200 bg-white">
                {(clientsData?.clients.rows ?? []).map((client) => (
                  <button
                    key={client.id}
                    type="button"
                    onClick={() => {
            ...
            {selectedClient && (
              <p className="mt-xs text-xs text-primary-700">
                Cliente selecionado: {selectedClient.fullName}
              </p>
            )}
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-md">
        ...
        <footer className="flex justify-end gap-xs px-md py-md border-t border-neutral-200 bg-neutral-50">
...
export function SchedulePage() {
  ...
  const [closingFilter, setClosingFilter] = useState<"all" | "open" | "closed">(
    "all",
  );
  ...
  const [modalSeed, setModalSeed] = useState<
    { hour: number; minute: number; profId?: string } | undefined
  >();
  ...
  const { data: appointmentsData, refetch: refetchAppointments } =
    useQuery<AppointmentsQueryResult>(AppointmentsQuery, {
      variables: {
        startsAt: range.from,
        endsAt: range.to,
        professionalId: null,
  ...
  const visibleProfs = useMemo(
    () =>
      professionals.filter((professional) =>
        visibleProfIds.has(professional.id),
      ),
    [professionals, visibleProfIds],
  );
  const dayAppointments = useMemo(
    () =>
      (appointmentsData?.appointments ?? [])
        .map(appointmentToSchedule)
        .filter(
          (appointment) =>
            visibleProfIds.has(appointment.professionalId) &&
    ...
    [appointmentsData, visibleProfIds, activeStatuses, searchQuery],
  );
    ...
    <div className="flex h-[calc(100vh-7rem)] lg:h-[calc(100vh-5rem)] -mx-md lg:-mx-lg -my-md lg:-my-lg">
      ...
      <div className="flex-1 flex flex-col min-w-0 bg-neutral-50/30">
        ...
        <div className="flex items-center gap-sm px-md py-sm border-b border-neutral-200 bg-white shrink-0 flex-wrap">
          ...
          <div className="flex items-center gap-xs ml-auto">
            <Button
              ...
              onClick={() => {
                const allVisible = visibleProfIds.size === professionals.length;
                if (allVisible) {
                  setVisibleProfIds(
                    new Set([professionals[0]?.id].filter(Boolean) as string[]),
                  );
                } else {
                  setVisibleProfIds(new Set(professionals.map((p) => p.id)));
                }
              }}
            >
              {visibleProfIds.size === professionals.length ? (
            ...
            <div className="hidden sm:flex items-center gap-xs px-sm py-xs rounded-md bg-warning-50 border border-warning-200">
              ...
              <span className="text-[11px] font-medium text-warning-700 leading-none">
                Pgto
                <br />
                Indica
              </span>
        ...
        <div className="flex-1 overflow-auto">
          <div className="bg-white border border-neutral-200 m-md rounded-lg overflow-hidden">
            ...
            >
              ...
              {visibleProfs.length === 0 ? (
                ...
                visibleProfs.map((pro) => (
                  ...
                  >
                    <div className="h-8 w-8 rounded-full bg-neutral-100 flex items-center justify-center text-neutral-500 shrink-0 overflow-hidden">
                      <div
            ...
            >
              ...
              {isToday &&
                nowLineTop > 0 &&
                nowLineTop < TIME_SLOTS.length * slotHeight && (
                  <div
        ...
        <footer className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-md px-md py-md border-t border-neutral-200 bg-white text-xs text-neutral-500">
          <nav
            className="flex flex-wrap items-center gap-x-md gap-y-xs"
            aria-label="Links institucionais"
          >
            <a className="hover:text-primary-500 transition-colors" href="#">
              Meu site
            </a>
